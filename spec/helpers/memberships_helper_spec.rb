...
describe MembershipsHelper, '#format_messenger' do
  it 'turns a skype name into a skype url' do
    expect(helper.format_messenger('Skype', 'test')).to eql('<a href="skype:test?add">test</a>')
  end

  it 'links to twitter' do
    expect(helper.format_messenger('Twitter', 'test')).to eql('<a href="http://twitter.com/test">@test</a>')
  end
